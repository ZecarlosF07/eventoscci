...
export function CertificatesManagementTemplate({ canViewAudit, data }: CertificatesManagementTemplateProps) {
  return <div className="space-y-7"><SectionHeading description={`${data.total} certificados históricos de eventos y capacitaciones.`} eyebrow="Emisión institucional" title="Certificados" /><nav className="flex flex-wrap gap-2"><Link className="rounded-xl bg-cci-950 px-4 py-2 text-sm font-semibold text-white" href={ROUTES.adminCertificatesActivities}>Emitir por actividad</Link><Link className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold" href={ROUTES.adminCertificateTemplates}>Plantillas y firmantes</Link>{canViewAudit ? <><Link className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold" href={ROUTES.adminCertificateAccess}>Accesos públicos</Link><Link className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold" href={ROUTES.adminCertificateQueries}>Consultas por DNI</Link></> : null}</nav><CertificatesTable certificates={data.certificates} /><Pagination page={data.page} pageCount={data.pageCount} pathname={ROUTES.adminCertificates} /></div>;
}
