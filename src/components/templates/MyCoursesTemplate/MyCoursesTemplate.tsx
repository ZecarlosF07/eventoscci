...
export function MyCoursesTemplate({ courses }: MyCoursesTemplateProps) {
  ...
  return (
    <div className="space-y-8">
      <header>
        <p className="text-sm font-semibold uppercase tracking-widest text-slate-500">Campus Virtual</p>
      ...
      {courses.length ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <Link className="mt-5 inline-flex rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white" href="/cursos">Ver catálogo</Link>
        </div>
      )}
    </div>
  );
