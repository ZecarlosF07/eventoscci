...
export async function ActivityDetailTemplate({ activity }: ActivityDetailTemplateProps) {
  ...
  return (
    <article className="py-10 sm:py-14">
      ...
      <header className="mt-6 grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-start">
        <div>
          ...
          <Heading className="mt-5" level={1}>{activity.title}</Heading>
          {activity.short_description ? <Text className="mt-4" size="lg">{activity.short_description}</Text> : null}
        </div>
      ...
      {bannerUrl ? <div className="relative mt-9 aspect-[16/7] overflow-hidden rounded-3xl bg-slate-100"><Image alt={`Banner de ${activity.title}`} className="object-cover" fill priority sizes="100vw" src={bannerUrl} /></div> : null}
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_0.4fr]">
        ...
        <aside className="space-y-5 rounded-3xl bg-slate-100 p-6 lg:sticky lg:top-6">
