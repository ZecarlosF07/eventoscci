    ...
    <header className="sticky top-0 z-40 border-b border-cci-100 bg-white/95 backdrop-blur">
      <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-5 px-5 sm:px-8">
        <div className="flex items-center gap-7">
          ...
          <nav aria-label="Navegación del Campus" className="hidden md:block"><NavigationLinks items={CAMPUS_NAVIGATION} variant="workspace" /></nav>
        </div>
        ...
        <details className="relative md:hidden"><summary className="flex min-h-11 cursor-pointer list-none items-center rounded-xl border border-cci-200 px-3 text-sm font-semibold marker:content-none">☰ Menú</summary><div className="absolute right-0 top-14 w-[min(20rem,calc(100vw-2.5rem))] rounded-2xl border border-cci-100 bg-white p-4 shadow-xl"><nav aria-label="Navegación móvil del Campus"><NavigationLinks items={CAMPUS_NAVIGATION} variant="workspace" /></nav><div className="mt-4 border-t border-cci-100 pt-4"><Text size="sm">{account.person.first_names} {account.person.last_names}</Text><form action={logoutAction} className="mt-3"><Button className="w-full" type="submit" variant="secondary">Cerrar sesión</Button></form></div></div></details>
      </div>
    </header>
    <main className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 sm:py-12">{children}</main>
  </div>;
}
