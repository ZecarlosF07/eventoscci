...
import { ParticipantCertificatesPanel } from "@/features/certificates/components/ParticipantCertificatesPanel";
...
export function ParticipantDetailTemplate({ participant }: ParticipantDetailTemplateProps) {
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4"><Heading level={2}>Certificados</Heading><Text>Guarda primero las correcciones del nombre. Después podrás reemplazar los documentos que todavía muestran el dato anterior.</Text><ParticipantCertificatesPanel certificates={participant.certificates} participantId={participant.id} participantName={`${participant.first_names} ${participant.last_names}`} /></section>
