...
export async function generateStaticParams() {
  ...
  return entries.activities
    .filter((activity) => activity.type === "training")
    .map((activity) => ({ slug: activity.slug }));
}

export async function generateMetadata({ params }: ActivityDetailPageProps): Promise<Metadata> {
  const activity = await getPublicActivityBySlug("training", (await params).slug);
  if (!activity) return buildNoIndexMetadata("Capacitación no encontrada");
...
export default async function TrainingDetailPage({ params }: ActivityDetailPageProps) {
  const activity = await getPublicActivityBySlug("training", (await params).slug);
