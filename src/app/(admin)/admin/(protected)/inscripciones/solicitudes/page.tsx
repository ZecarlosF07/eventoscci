...
export default async function MemberGroupRequestsPage({ searchParams }: PageProps) {
  ...
  return <div className="space-y-7">
    ...
    <div className="space-y-3">{data.items.map((item) => <Link key={item.id} href={`/admin/inscripciones/solicitudes/${item.id}`} className="block rounded-2xl border border-cci-100 bg-white p-5 transition hover:border-cci-500 hover:shadow-sm focus-visible:outline-2 focus-visible:outline-cci-700">
      ...
      <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm"><span>{item.confirmed_count}/{item.seat_count} plazas confirmadas</span><span>Total S/ {item.total.toFixed(2)}</span><strong className="text-amber-800">Pendiente S/ {item.pending_amount.toFixed(2)}</strong><span>{item.billing_type === "factura" ? `Factura a RUC ${item.billing_document}` : item.billing_type === "boleta" ? "Boleta" : "Gratuito"}</span></div>
    </Link>)}</div>
