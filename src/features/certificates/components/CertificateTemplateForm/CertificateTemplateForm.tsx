...
export function CertificateTemplateForm({ template }: CertificateTemplateFormProps) {
  ...
  return <form action={action} className="space-y-6 rounded-3xl border border-cci-100 bg-white p-6" encType="multipart/form-data">
    ...
    <div className="space-y-4"><h2 className="text-lg font-semibold">Firmantes</h2><p className="text-sm text-slate-600">Puedes configurar hasta tres firmantes por plantilla.</p>{Array.from({ length: 3 }, (_, index) => { const signer = signers[index]; return <div className="grid gap-4 rounded-2xl bg-cci-50 p-4 md:grid-cols-3" key={index}><input name={`existing_signature_path_${index}`} type="hidden" value={signer?.signature_path ?? ""} /><FormField label={`Firmante ${index + 1}`} name={`signer_name_${index}`}><Input defaultValue={signer?.signer_name ?? ""} id={`signer_name_${index}`} name={`signer_name_${index}`} /></FormField><FormField label="Cargo" name={`signer_title_${index}`}><Input defaultValue={signer?.signer_title ?? ""} id={`signer_title_${index}`} name={`signer_title_${index}`} /></FormField><FormField label="Firma" name={`signature_${index}`}><Input accept="image/png,image/jpeg,image/webp" id={`signature_${index}`} name={`signature_${index}`} type="file" /></FormField></div>; })}</div>
