...
export function MaterialForm({ courseId, material }: MaterialFormProps) {
  ...
  return <form className="space-y-4 rounded-2xl border border-cci-100 bg-white p-5" method="post" onSubmit={onSubmit}>
    ...
    <div className="flex items-center justify-between gap-3"><FormActionNotice compact message={state.message} success={state.success} /><Button disabled={pending} type="submit">{pending ? "Guardando…" : material ? "Actualizar" : "Agregar material"}</Button></div>
  </form>;
}
