...
export function CourseForm({ course, speakers }: CourseFormProps) {
  const [state, action, pending] = useActionState(saveCourseAction, INITIAL_STATE);
  ...
  return <form action={action} className="space-y-6">
    ...
    <ActivityFormSection title="Información general">
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FormField hint="Déjalo vacío para generarlo desde el título." label="Slug" name="slug"><Input defaultValue={course?.slug} id="slug" name="slug" /></FormField>
      </div>
    ...
    <ActivityFormSection title="Instructores"><CourseInstructorFields initialInstructors={initialInstructors} speakers={speakers} /></ActivityFormSection>
    ...
    <div className="sticky bottom-4 flex items-center justify-between gap-4 rounded-2xl border border-cci-100 bg-white/95 p-4 shadow-lg backdrop-blur">
      <p className={state.success ? "text-sm text-amber-800" : "text-sm text-rose-700"}>{state.message}</p>
