...
export function LessonForm({ courseId, defaultSortOrder = 0, lesson, moduleId }: LessonFormProps) {
  ...
  return (
    <form className="space-y-5" method="post" onSubmit={onSubmit}>
      ...
      <div className="grid gap-5 md:grid-cols-[minmax(0,1fr)_9rem]"><FormField error={state.errors?.title?.[0]} hint={`Máximo ${FIELD_LIMITS.contentTitle} caracteres.`} label="Título de la clase" name={`lesson_title_${fieldSuffix}`} required><Input defaultValue={lesson?.title} id={`lesson_title_${fieldSuffix}`} maxLength={FIELD_LIMITS.contentTitle} name="title" placeholder="Ej. Introducción y objetivos" required /></FormField><FormField error={state.errors?.sortOrder?.[0]} hint="0 aparece primero." label="Posición" name={`lesson_order_${fieldSuffix}`}><Input defaultValue={lesson?.sort_order ?? defaultSortOrder} id={`lesson_order_${fieldSuffix}`} min="0" name="sort_order" type="number" /></FormField></div>
