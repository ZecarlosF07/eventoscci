...
export function ModuleForm({ courseId, defaultSortOrder = 0, module }: ModuleFormProps) {
  ...
  return (
    <form className="space-y-5" method="post" onSubmit={onSubmit}>
      ...
      <FormField error={state.errors?.title?.[0]} hint={`Máximo ${FIELD_LIMITS.contentTitle} caracteres.`} label="Título del módulo" name={`module_title_${fieldSuffix}`} required><Input defaultValue={module?.title} id={`module_title_${fieldSuffix}`} maxLength={FIELD_LIMITS.contentTitle} name="title" placeholder="Ej. Fundamentos de gestión comercial" required /></FormField>
