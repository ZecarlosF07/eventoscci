...
export function ActivityLocationMap({ activity }: ActivityLocationMapProps) {
  if (
    ...
    !activity.maps_embed_url ||
    !isGoogleMapsEmbedUrl(activity.maps_embed_url)
  ) return null;

  const directionsUrl = getGoogleMapsDirectionsUrl(activity.location_name, activity.address);
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <Heading level={2}>Cómo llegar</Heading>
          {activity.location_name ? <Text className="mt-2">{activity.location_name}</Text> : null}
          {activity.address ? <Text size="sm">{activity.address}</Text> : null}
        </div>
      ...
      <div className="mt-5 aspect-[4/3] overflow-hidden rounded-2xl bg-cci-100 ring-1 ring-cci-200 sm:aspect-[16/7]">
        <iframe allowFullScreen className="h-full w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={activity.maps_embed_url} title={`Mapa de ${activity.title}`} />
      </div>
    </section>
  );
}
