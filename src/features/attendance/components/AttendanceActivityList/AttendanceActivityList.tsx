    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {activities.map((activity) => (
        <article className="flex flex-col justify-between gap-5 rounded-3xl border border-slate-200 bg-white p-6" key={activity.id}>
          ...
          <Link className="inline-flex min-h-11 items-center justify-center rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white hover:bg-slate-800" href={`${ROUTES.adminAttendance}/${activity.id}`}>Gestionar asistencia</Link>
        </article>
      ))}
    </div>
  );
