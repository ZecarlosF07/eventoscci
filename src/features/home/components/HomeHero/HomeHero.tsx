  ...
  return (
    <section className="overflow-hidden bg-cci-950 text-white">
      <div className="mx-auto grid max-w-[90rem] lg:min-h-[34rem] lg:grid-cols-[0.92fr_1.08fr]">
        <div className="flex flex-col justify-center px-4 pb-12 pt-8 sm:px-8 sm:py-10 lg:px-12 lg:py-14">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-cci-lime sm:text-sm sm:tracking-[0.2em]">
            {activity ? "Próxima actividad" : "Eventos y formación CCI"}
          </p>
          <h1 className="mt-3 max-w-2xl text-3xl font-semibold leading-[1.04] tracking-tight sm:text-5xl lg:text-[3.5rem]">
            {activity ? activity.title : "Conecta, aprende y haz crecer tu empresa"}
          </h1>
          <p className="mt-3 line-clamp-2 max-w-xl text-sm leading-6 text-white/72 sm:mt-4 sm:text-lg sm:leading-7">
          ...
          <div className="mt-5 flex flex-wrap gap-3 sm:mt-7">
            <Link
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cci-lime px-5 text-sm font-bold text-cci-950 transition hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white sm:min-h-12"
          ...
          {!bannerUrl && nextDateValue ? (
            <div className="mt-6 flex items-center gap-3 border-l border-cci-lime/45 pl-4 lg:hidden">
              <span aria-hidden="true" className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-cci-lime text-cci-950">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-base font-semibold text-white">{HERO_DATE_FORMATTER.format(nextDateValue)}</p>
                <p className="mt-0.5 text-sm text-white/65">A las {HERO_TIME_FORMATTER.format(nextDateValue)}</p>
              </div>
            </div>
          ) : null}
        </div>
        <div className={`relative overflow-hidden lg:min-h-full ${bannerUrl ? "min-h-[14rem] bg-cci-900 sm:min-h-[19rem]" : "hidden bg-cci-950 lg:block"}`}>
          {bannerUrl ? (
            <>
          ...
          {bannerUrl && nextDateValue ? (
            <div className="absolute left-5 top-5 flex items-center gap-4 rounded-2xl border border-white/15 bg-cci-950/88 p-3.5 shadow-xl backdrop-blur-md sm:left-6 sm:top-6 sm:p-4">
              ...
              <div className="min-w-0">
                <p className="text-[0.68rem] font-bold uppercase tracking-[0.18em] text-cci-lime">Próxima fecha</p>
                <p className="mt-1 text-sm font-semibold text-white sm:text-base">{HERO_DATE_FORMATTER.format(nextDateValue)} · {HERO_TIME_FORMATTER.format(nextDateValue)}</p>
              </div>
            </div>
          ) : null}
        </div>
