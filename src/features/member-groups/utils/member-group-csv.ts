...
export function memberGroupsToCsv(groups: MemberGroupAdminListItem[], seats: MemberGroupExportSeat[]): string {
  ...
  const headers = ["Código solicitud", "Actividad", "RUC asociado", "Empresa asociada", "Comprobante", "Documento facturación", "Código asistente", "Documento asistente", "Nombres", "Apellidos", "Correo", "Estado", "Precio individual", "Total solicitud (una vez)", "Importe confirmado (una vez)", "Saldo pendiente (una vez)", "Fecha solicitud"];
