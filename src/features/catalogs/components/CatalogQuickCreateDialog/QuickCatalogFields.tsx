...
export function QuickCatalogFields({ errors, kind }: QuickCatalogFieldsProps) {
  ...
  if (kind === "venues") return (
    <div className="grid gap-4">
      <FormField error={error("name")} label="Nombre del lugar" name="name" required><Input id="quick-name" maxLength={FIELD_LIMITS.venueName} name="name" required /></FormField>
  ...
  if (kind === "contacts") return (
    <div className="grid gap-4">
      <FormField error={error("label")} label="Nombre identificador" name="label" required><Input id="quick-label" maxLength={FIELD_LIMITS.activityContactLabel} name="label" placeholder="Eventos CCI" required /></FormField>
      ...
      <FormField error={error("whatsapp_phone")} label="WhatsApp" name="whatsapp_phone" required><Input id="quick-whatsapp" maxLength={FIELD_LIMITS.phone} name="whatsapp_phone" required type="tel" /></FormField>
  ...
  if (kind === "categories") return (
    <div className="grid gap-4">
      <FormField error={error("name")} label="Nombre" name="name" required><Input id="quick-category-name" maxLength={FIELD_LIMITS.categoryName} name="name" required /></FormField>
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <FormField error={error("first_names")} label="Nombres" name="first_names" required><Input id="quick-first-names" maxLength={FIELD_LIMITS.personName} name="first_names" required /></FormField>
      <FormField error={error("last_names")} label="Apellidos" name="last_names" required><Input id="quick-last-names" maxLength={FIELD_LIMITS.personName} name="last_names" required /></FormField>
