...
begin
  ...
  if v_id is not null then
    ...
    from public.certificate_templates where id = v_id and deleted_at is null for update;
  end if;

  if v_old is null then
    v_id := coalesce(v_id, gen_random_uuid());
    insert into public.certificate_templates (
      id, name, scope, background_path, template_config, is_default, is_active
    ) values (
      v_id, v_name, v_scope, v_background_path, v_template_config, false, v_is_active
    );
  else
    update public.certificate_templates set
      name = v_name, scope = v_scope, background_path = v_background_path,
      template_config = v_template_config, is_default = false, is_active = v_is_active
    where id = v_id;
  ...
  update public.certificate_templates set is_default = v_is_default and v_is_active where id = v_id;

  for v_signer in select value from jsonb_array_elements(p_signers)
  loop
    if length(btrim(v_signer->>'signer_name')) < 3 or length(btrim(v_signer->>'signer_name')) > 200
    then raise exception 'VALIDATION_ERROR' using errcode = '22023'; end if;
    insert into public.certificate_template_signers (
      template_id, signer_name, signer_title, signature_path, sort_order
    ) values (
      v_id, btrim(v_signer->>'signer_name'), nullif(btrim(v_signer->>'signer_title'), ''),
      nullif(btrim(v_signer->>'signature_path'), ''), coalesce((v_signer->>'sort_order')::integer, 0)
    );
  end loop;

  insert into public.audit_logs (actor_user_id, action, entity_type, entity_id, old_data, new_data)
  values (
    auth.uid(), case when v_old is null then 'certificate_template.created' else 'certificate_template.updated' end,
    'certificate_template', v_id, v_old,
